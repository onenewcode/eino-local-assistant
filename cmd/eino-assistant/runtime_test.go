...
func TestMCPServerOptionsFilterDisabledAndPreserveConfiguration(t *testing.T) {
	...
	servers := []config.MCPServerConfig{
		...
		{Name: "remote", Type: config.MCPTransportStreamableHTTP, URL: "https://mcp.example.test/v1", OAuth: true, ConnectTimeoutSeconds: 8},
	}
	...
	if options[2].Name != "remote" || options[2].Type != config.MCPTransportStreamableHTTP || options[2].URL != "https://mcp.example.test/v1" || !options[2].OAuth || options[2].ConnectTimeout != 8*time.Second {
