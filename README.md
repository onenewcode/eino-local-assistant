...
- **CLI 子命令**：`chat` / `resume` / `sessions` / `mcp` / `completion` / `init` / `export` / `version`（默认无子命令即 chat）

默认无子命令、`chat`/`new` 与 `resume` 需要交互式终端；`exec`、`sessions`、`mcp`、`completion`、`init`、`export`、`version` 与 `help` 可在管道或非 TTY 环境运行。管道/非 TTY 输入进入 TUI 会直接报错退出。当前不包含跨会话向量检索或多 agent worker。
