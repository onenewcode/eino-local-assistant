...
- **后台分析子 agent（安全子集）**：TUI `/agent <task>` 启动最多 4 个 process-local、只读且无工具的分析 child；`/agents` 可列出、查看有界 display-only 结果或按 ID 取消，结果不会自动写回父 session、usage 或模型上下文
