...
# 显式危险模式：跳过 approval 与 OS sandbox（仅交互 TUI）
go run ./cmd/eino-assistant --yolo
go run ./cmd/eino-assistant resume 20260715-120000-abc123 --yolo
...
`exec` 复用普通会话的权限、sandbox、ReAct 和 runtime guard 接线，但不接受 `--yolo`；headless 没有 TUI approver，继续按配置中的静态 `approval_policy` 处理。`--yolo` 只属于交互 TUI，不能通过 headless 参数或 prompt 静默开启。
...
| `/tasks` | 有界只读资源摘要：foreground turn 状态、当前工具（清洗并截断）、queued follow-up 数量、`/goal`/`Ctrl+T` 投影可用性，以及明确的 background shell/subagent runtime unavailable |
...
| `/permissions [ask\|auto\|plan\|yolo]` | 权限规则、sandbox 模式/后端/网络、运行时预算与本 session 决策；模式切换只在 idle 生效；yolo 为显式危险 host bypass，不在 Shift+Tab |
...
工具调用显示为 Claude/Codex 风格卡片（`⚙ name` + 缩进 `⎿` 结果，JSON 会 pretty-print）；assistant 回复在完成后对 markdown/代码块做终端渲染（流式阶段为纯文本）。idle 状态栏显示 `provider/model` / 短 session id / API usage / `cost~` / 可选 context 快照；复杂任务显示紧凑的 `task:n/m` 进度，`Ctrl+T` 可展开只读的状态、目标、范围、当前工作与 gap，`/tasks` 只显示有界的 foreground/resource 摘要，busy 状态栏还会显示当前工具名与 `queued:N`。上滚离开底部时状态栏提示 `↑ End to follow`。每轮完成后会显示该轮的 `API usage: prompt / generation / total / calls`，不会把 API 请求输入误称为用户输入。

生成或压缩中可立即运行 `/help`、`/context`、`/status`、`/goal`、`/tasks`、`/rules`、`/btw`、`/side`、`/sessions`、`/queue`、`/permissions` 与只读 `/memory` 操作；这些命令不调用模型、工具或 shell，`/tasks` 还不写 session ledger、不改变 turn/queue。`/tasks` 只观察 foreground turn，并明确报告当前实现没有 background shell/subagent runtime；`/goal` 与 `Ctrl+T` 才是 goal/checklist 投影，不从它们推断后台资源，也没有后台进程的 stop/resume 控制。`/plan` 的无参、prompt 和 `exit|ask|auto` 形式，以及 `/permissions ask|auto|plan|yolo`，在 busy、compacting、pending approval 或 side question 时立即拒绝、不排队、不取消当前操作并保留 composer draft。`/permissions yolo` 只在 idle 显式开启 host bypass，且不改变 hard deny/path safety。`/queue clear` 会立刻丢弃未开始的 follow-up。自然语言仍按 FIFO 排队；旁路问题不进入该队列，也不打断当前 turn，多个旁路问题可以并发执行。busy 时 `Esc` / `Ctrl+C` 中断当前 turn；backtrack requires an empty composer，idle 且 composer 非空时 `Esc` 保持草稿且不 arm/open，composer 为空时连续两次 `Esc` 进入 backtrack 选择。`/compact`、`/clear`、`/new`、`/resume`、`/fork`、`/title`、`/delete`、`/exit` 等变更性命令须等 idle；`/fork` 也不会进入 FIFO。
...
- 每个 session 含 `journal.jsonl`（权威事件链）、`system_prompt.txt`（冻结的 system prompt）、`state.json` / `meta.json`（可重建投影）、`checkpoints/`、`artifacts/` 和 `locks/write.lock`。
- 每个 turn 会记录 `turn.started`、完整 tool 生命周期、`turn.committed` / `turn.cancelled` / `turn.failed`。取消和失败不会进入后续模型 prompt，但保留审计记录。
- tool 结果按 session 生命周期记录：原始工具参数不写入 durable journal，短结果直接 inline，只有超过 16 KiB 的结果才写入内容寻址 artifact；默认单一 artifact 至 4 MiB、每 session 至 64 MiB。后续 agent 可通过 `read_artifact` 按范围读取同一 session 的证据；超限保留 SHA-256、原始大小、head/tail 摘要和 `truncated=true`。
