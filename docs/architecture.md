...
SQLite 分层更新上述存储细节：ephemeral resume 不再复制 `state.json` / `meta.json`，而是只复制目标 session 的 journal、system prompt、checkpoints 与 artifacts，并在临时根目录建立独立投影；durable 全局 `state.sqlite3` 不复制。ephemeral `--last` 通过只读 store 选择，不创建、迁移、修复数据库或产生 WAL。runtime `Close` 先关闭 session/memory 数据库，再删除 ephemeral 根目录。
...
| `internal/store` | canonical session journal、可重建的 `<data_dir>/state.sqlite3` 历史投影、checkpoint、artifact、resume，以及 source-preserving fork primitives | 项目记忆；provider 构造；catalog/picker；全局配置重写；TUI 命令编排；workspace、git 或外部副作用回滚 |
...
| 会话账本 | `store` + `chat` | 可 `/resume` | compact journal、system_prompt.txt、artifact 与 checkpoint 是真相 |
