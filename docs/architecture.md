...
SQLite 分层更新上述存储细节：ephemeral resume 只复制选定的 `sessions/YYYY/MM/DD/<id>/` 会话包，并在临时根目录建立独立投影；durable 全局 `state.sqlite3` 不复制。ephemeral `--last` 通过只读 store 选择，不创建、迁移、修复数据库或产生 WAL。runtime `Close` 先关闭 session/memory 数据库，再删除 ephemeral 根目录。
...
| `internal/store` | canonical `updates.jsonl` ledger、每会话 `summary.json` 投影、可重建的 `<data_dir>/state.sqlite3` 历史投影、checkpoint、artifact、resume，以及 source-preserving fork primitives；active session 固定为 `sessions/YYYY/MM/DD/<id>/` | 项目记忆；provider 构造；catalog/picker；全局配置重写；TUI 命令编排；workspace、git 或外部副作用回滚 |
