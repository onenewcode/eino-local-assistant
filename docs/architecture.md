...
TUI 会话内权限模式为 `ask`、`auto`、`plan` 和显式 `yolo`。`plan` 是 TUI 进程级临时 read-only phase，不是配置持久化，也不是完整的 plan artifact workflow；它不生成或持久化任务计划、计划文件或独立 plan 账本。`/permissions` 无参数是只读报告；`/permissions plan` 只在 idle 时进入，`/permissions ask` 与 `/permissions auto` 只在 idle 时退出 plan，busy、compacting、pending approval 或 side question 时拒绝且不进入 FIFO，并保留 composer draft。`auto` 只在 `internal/tools` 的实际授权边界自动处理 `DecisionAsk`。plan 下 `apply_patch` 无条件返回结构化 `plan_read_only` 拒绝；shell 除了要求 `PermissionSet` 原始 `DecisionAllow` 外，还只接受受控的只读检查命令（`pwd`、`ls`、`find` 的非执行形式、`rg`、`grep`、`cat`、`head`、`tail`、`git status/diff/log`）进入 enforced OS read-only worker，hard deny、含副作用或不明确的命令仍 fail-closed，其他非原始 allow 不进入 ask；缺少或无法证明该 sandbox 时 fail-closed，不无 sandbox 执行，也不请求 host escalation。所有模式仍受硬 deny、workspace/path clamp、sandbox 与 host escalation 上限约束。模式不写配置、session ledger 或 resume 数据。

显式 yolo 的运行时接线由 `cmd/eino-assistant` 创建 `tools.ApprovalState`，由 `internal/tools` 同时跳过普通 approval 和 OS sandbox worker；它不是 prompt 层的状态，也不由静态 `approval_policy` 配置启用。启动入口为交互 TUI 的 `--yolo`（裸命令、`chat`/`new`、`resume`），运行中入口为 idle-only `/permissions yolo`。yolo 不加入 `Shift+Tab` 的 `ask -> auto -> plan -> ask` 循环；状态、警告、报告和工具 `SandboxOutcome` 由 `internal/tui` 与工具层暴露。`PermissionSet` hard deny、shell workspace working directory、`apply_patch` workspace/path/symlink 检查仍在工具边界执行，不能被模型 prompt 绕过；yolo 是显式 host bypass，不是普通 sandbox 不可用时的 fallback。headless `exec` 拒绝 `--yolo`，yolo 不写 TOML、session ledger 或 resume 数据。
...
| `internal/tools` | shell、apply_patch、artifact、memory 只读工具；权限规则求值及 ask/auto/plan/yolo 在工具授权边界的应用 | TUI 策略展示；配置与 TUI 命令编排 |
...
| TUI 会话权限模式 | `internal/tui` 的命令/idle admission、`Shift+Tab` 循环与展示；`cmd/eino-assistant` runtime 接线；`internal/tools` 在 shell/apply_patch 授权边界执行 | 当前 TUI 进程；不进入 config、ledger 或 resume | `ask`、`auto`、`plan`、`yolo`；plan 是临时 read-only phase，不是完整 plan artifact workflow；apply_patch 无条件结构化拒绝，shell 仅原始 PermissionSet allow 且属于受控只读检查集合时进入 enforced OS read-only worker；hard deny、path clamp、sandbox 与 host escalation 上限仍有效 |
...
| 显式 yolo 权限模式 | `cmd/eino-assistant`（`--yolo` 交互入口与 runtime 状态）+ `internal/tui`（idle `/permissions yolo`、警告、状态与审计展示）+ `internal/tools`（approval/sandbox 双 bypass 与 hard deny/path safety）；不新增 config/store 持久层，headless `exec` 拒绝 |
