...
本文件只回答“改什么该去哪里”。代码是行为细节的最终真相；产品对齐应直接核验其公开文档或源码，避免把调研过程当作产品文档。
...
SQLite 是可丢弃的 session catalog/resume index，不镜像 event、message、tool output、usage 或 checkpoint payload。它只保存当前有界 state、JSONL head identity，以及 checkpoint/tail 的 byte locator；任一 schema、锁或读取错误都退回 JSONL，不能阻断 resume 或决定下一条 hash。正常长会话恢复只读取“最新已验证 checkpoint + 未覆盖 tail”；JSONL 损坏、offset 失效或索引缺失时才流式全量重放。ephemeral resume 只复制选定的 `sessions/YYYY/MM/DD/<id>.jsonl` 会话账本，并在临时根目录建立独立投影；durable 全局 `state.sqlite3` 不复制。ephemeral `--last` 通过只读 store 选择，不创建、迁移、修复数据库或产生 WAL。runtime `Close` 先关闭 session/memory 数据库，再删除 ephemeral 根目录。
...
TUI 会话内权限模式为 `ask`、`auto`、`plan` 和显式 `yolo`。`plan` 是 TUI 进程级临时 read-only phase，不是配置持久化，也不是完整的 plan artifact workflow；它不生成或持久化任务计划、计划文件或独立 plan 账本。`/permissions` 无参数是只读报告；`/permissions plan` 只在 idle 时进入，`/permissions ask` 与 `/permissions auto` 只在 idle 时退出 plan，busy、compacting、pending approval 或 side question 时拒绝且不进入 FIFO，并保留 composer draft。`auto` 只在 `internal/tools` 的实际授权边界自动处理普通 approval fallback；Codex `prompt` 规则仍必须得到人类确认。plan 下 `apply_patch` 无条件返回结构化 `plan_read_only` 拒绝；shell 只有在 Codex 规则/已知安全回退为 allow、影响分类为 `read_only`，且存在实际 enforced 的 OS 只读 sandbox 时才进入 worker。匹配到的 `forbidden`、含副作用或不明确的命令仍 fail-closed；缺少或无法证明该 sandbox 时 fail-closed，不无 sandbox 执行，也不请求 host escalation。yolo 例外地跳过 OS sandbox 和普通审批，剩余规则、路径和字符串级命令检查只是尽力而为，不能作为宿主安全保证。模式不写配置、session ledger 或 resume 数据。

显式 yolo 的运行时接线由 `cmd/eino-assistant` 创建 `tools.ApprovalState`，由 `internal/tools` 同时跳过普通 approval 和 OS sandbox worker；它不是 prompt 层的状态，也不由静态 `approval_policy` 配置启用。启动入口为交互 TUI 的 `--yolo`（裸命令、`chat`/`new`、`resume`），运行中入口为 idle-only `/permissions yolo`。yolo 不加入 `Shift+Tab` 的 `ask -> auto -> plan -> ask` 循环；状态、警告、报告和工具 `SandboxOutcome` 由 `internal/tui` 与工具层暴露。规则匹配、shell workspace working directory、`apply_patch` workspace/path/symlink 和字符串级命令检查仍会运行，但 yolo 下没有 OS 隔离，不能把它们视为完整 shell 解析或宿主安全边界；yolo 是显式 host bypass，不是普通 sandbox 不可用时的 fallback。headless `exec` 拒绝 `--yolo`，yolo 不写 TOML、session ledger 或 resume 数据。

`/plan` 与 `/permissions plan` 进入同一个临时 read-only phase，复用相同的 idle-only admission、状态展示与执行边界；`Shift+Tab` 在同一 admission 下显式循环 `ask -> auto -> plan -> ask`，不会隐式开启任何更高权限。它不新增 artifact、持久化或权限语义。`/plan` 无参数只切换到 plan；`/plan <prompt>` 仅在 idle 时先切到 plan，再通过同一正常 TUI turn 路径执行一次 prompt，并在 turn 结束后保持 plan；精确的 `/plan exit` 与 `/plan ask` 恢复 ask，`/plan auto` 恢复 auto。所有 `/plan` 形式和 `Shift+Tab` 在 busy、compacting、pending approval 或 side question 时立即拒绝、不排队、不取消当前操作并保留 composer draft；prompt 启动失败时不启动 turn，模式仍保持 plan。这仍不是持久 plan workflow。

Headless `exec` 不使用该 TUI 状态，也没有 `/permissions` 交互命令；其 `approval_policy` 继续是启动时静态语义，默认 execute 行为不变。`on-request` 在没有交互 approver 时对需要审批的请求 fail-closed；`never` 拒绝 Codex `prompt` 规则，普通未命中审批回退仍只能在 sandbox 边界内自动处理。该 TUI 切换不改变 headless 行为。
...
| `internal/store` | canonical `sessions/YYYY/MM/DD/<id>.jsonl` ledger、可重建的 `<data_dir>/state.sqlite3` catalog/resume index、checkpoint、artifact、resume，以及 source-preserving fork primitives | 项目记忆；provider 构造；catalog/picker；全局配置重写；TUI 命令编排；workspace、git 或外部副作用回滚 |
| `internal/contextbuild` | prompt 规划与 compaction | 工具执行 |
| `internal/memory` | 项目级权威 `memory.sqlite3`、有界内存渲染与带 lease 的 consolidation | `/resume`、权限 |
| `internal/tools` | shell、apply_patch、artifact、memory 只读工具；嵌入式默认命令影响规则；权限规则求值及 ask/auto/plan/yolo 在工具授权边界的应用 | TUI 策略展示；配置与 TUI 命令编排 |
| `internal/sandbox` | 工具的 OS 隔离边界 | prompt 规则 |
| `internal/config` | 运行 TOML 读取、默认值与校验；静态 `approval_policy`；`model.catalog` 的显式 label/alias/lifecycle/capability 声明；可选的 provider/model-specific opaque `model.reasoning_effort` | TUI 会话运行时状态；provider discovery/health；会话内模型切换不重写全局配置 |
| `internal/tui` | 交互、slash、本地 FIFO 队列（列表、`drop`、`edit`、`clear`、`resume`）、显式 `/steer` 命令入口、审批桥、idle-only `/permissions ask|auto|plan|yolo`、`Shift+Tab` 安全模式循环与 `/plan [<prompt>|exit|ask|auto]` 命令入口、idle-only `/model [name]` 命令入口、显式 catalog picker 与 `Alt+P`、状态栏与权限模式展示、只读任务进度与 `/goal` 投影、只读 `/tasks` foreground/resource 摘要、只读 `/diff` Git 快照展示、idle-only `/review` slash/admission/异步 display-only workspace review（不将 review 当作 verify）、side-only 旁路结果展示/并发调度、idle-only `/fork` session 切换，以及 Esc backtrack selector/session 切换 | 模型协议、provider 构造、工具授权真相、旁路请求与 review 的模型调用、provider discovery/health/capability enforcement、Git 读取 callback 的进程边界与 workspace/Git 回滚、background shell/subagent runtime；队列不持久化，steer 核心语义由 `chat` / `agent` 负责 |
| `cmd/eino-assistant` runtime / headless output | 共享 runtime 接线（包括 TUI 与 side-effecting tools 的同一会话模式状态）、provider 构造、启动时模型 override、显式 yolo 的交互启动接线、catalog alias 解析、TUI idle model replacement 与 runtime-owned resume 的模型身份选择、picker DTO 接线、workspace diff callback 和 display-only workspace review 的一次无工具模型调用/接线、旁路问题的一次性只读模型调用，以及 `exec` 的 text/json/stream-json 投影、`--output-schema` 文件预加载与最终响应 delivery | provider structured-output 请求、ReAct 中间响应 schema、session/store schema、provider discovery/health、headless 的动态 TUI 模式 |
...
| 面 | 所有者 / 位置 | 生命周期 | 关键边界 |
| --- | --- | --- | --- |
| 执行安全边界 | `tools` + 启用的 `sandbox` | 进程 / 每次工具调用 | 不依赖 AGENTS.md 或 memory 文本执行；Codex `.rules` 是授权层，常规模式由 sandbox 和工具路径校验提供强制边界。yolo 绕过 sandbox，剩余字符串级命令检查不是宿主安全保证 |
| 工具授权与命令影响 | `internal/tools/rules/default.rules`（编译进二进制）+ `internal/tools` | 首次启动只初始化 `~/.eino-assistant/rules/default.rules`；该 Eino 自有模板零授权、不会复制 Codex 用户配置；读取用户 `~/.eino-assistant/rules/*.rules`，以及显式信任后读取 `<workspace>/.eino-assistant/rules/*.rules` | Codex-compatible Starlark `.rules` 只决定 allow/prompt/forbidden；`read_only`、`workspace_write`、`external_side_effect` 是独立派生分类，不授予权限；sandbox 才是执行边界。未知 shell 语法保守归为可写，复合命令取最高级别；详见 `docs/tool-policy.md` |
| TUI 会话权限模式 | `internal/tui` 的命令/idle admission、`Shift+Tab` 循环与展示；`cmd/eino-assistant` runtime 接线；`internal/tools` 在 shell/apply_patch 授权边界执行 | 当前 TUI 进程；不进入 config、ledger 或 resume | `ask`、`auto`、`plan`、`yolo`；plan 是临时 read-only phase，不是完整 plan artifact workflow；apply_patch 无条件结构化拒绝，shell 仅同时被 Codex 规则/已知安全回退允许且归为 read_only 的调用才进入 enforced OS read-only worker；yolo 是不安全的 host bypass，只有尽力而为的规则、路径和字符串检查 |
| 会话账本 | `store` + `chat` | 可 `/resume` | 单一 JSONL journal 及其 event payload 是真相；SQLite 仅为可重建投影 |
| 会话 fork / backtrack | `store` + `chat` + `tui` | `/fork` 仅 idle、无参数；backtrack 为 idle 两阶段 `Esc`，child 成功打开后切换 active session | store 复制 committed 前缀、重建 journal hash/seq、记录 parent provenance 并带入前缀 inline artifact；首个 prompt 使用显式 before-first 空 prefix，后续 prompt 在前一 committed turn 前 fork 并只回填 composer；拒绝 active/pending compaction/checkpoint/task-derived 状态；不实现 destructive rewind 或 source/workspace/Git/外部副作用回滚 |
...
- 任务图不增加修改、取消或完整 DAG slash 命令；`/goal` 只读显示有限的目标/状态/计数/进度/当前任务/PlanRequired/gaps 投影，状态栏显示紧凑进度，`Ctrl+T` 展开只读任务面板。`/tasks` 是独立的后台资源观察边界：它只摘要 foreground turn 与后台资源可用性，不把 goal、checklist 或 foreground turn 冒充成 background shell/subagent，也不宣称后台进程可停止或恢复；当前没有 background runtime 时固定显示 unavailable。
...
| source-preserving thread fork V1 | `internal/store` + `internal/chat` + `internal/tui`；用户命令入口在 TUI，cmd 层不单独解析同名 CLI 命令；普通空边界是 latest，before-first 是独立 API |
...
| 新工具或权限语义 | `internal/tools` + `sandbox` |
| 工具规则与命令影响 | `internal/tools`；用户 `~/.eino-assistant/rules/*.rules` 与显式信任的项目 `<workspace>/.eino-assistant/rules/*.rules` 使用 Codex Starlark 授权层，信任记录在用户 `~/.eino-assistant/config.toml` 的 `[projects]`；内置分类器为独立影响层；不写入 `AGENTS.md` 或持久会话配置 |
...
| 显式 yolo 权限模式 | `cmd/eino-assistant`（`--yolo` 交互入口与 runtime 状态）+ `internal/tui`（idle `/permissions yolo`、警告、状态与审计展示）+ `internal/tools`（approval/sandbox 双 bypass；规则和路径检查仍运行但不是宿主安全边界）；不新增 config/store 持久层，headless `exec` 拒绝 |
| 项目记忆 | `internal/memory` |
| slash、队列、状态展示 | `internal/tui` |
| 配置字段 | `internal/config` + `config.example.toml` |

改动包边界、持久化分层或安全边界时，同步更新本文件与仍存在的用户文档。代码是最终真相。
