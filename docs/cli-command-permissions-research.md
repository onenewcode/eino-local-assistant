...
对本仓库（`run_command` → `sh -c`，默认开启、无审批、无沙箱，仅有 timeout 与输出上限）的一句话评价：

> 当前只有第 0 层（可 `disabled`）和第 5 层的一部分（timeout/cap/杀进程组）。**缺少权限模型本体**。
...
| allow/deny | 无 |
| 沙箱 | 无 |
| cwd 限制 | 可配置默认 cwd；模型可传 `working_dir` |
| timeout | 默认 60s，上限 300s |
| 输出上限 | 默认 64KiB/流 |
| 取消 | Esc/父 ctx 取消 + 进程组 kill |
| 审计 | 工具生命周期进 thread journal（命令文本可见） |
| 用户可见模式 | 无 `policy=` 状态 |

代码入口：`internal/tools/command.go`。
...
#### Phase P0 — 文档与威胁说明（可与本文一并完成）

- README 明确：无审批、无沙箱、权限=启动用户。
- 本文作为设计依据。

#### Phase P1 — 最小权限闭环（高价值）

- policy 引擎：deny/ask/allow；
- 默认 `on_request` 或“allow 小集合 + 默认 ask”；
- TUI 审批 once/session；
- 工具结果带 `decision`；
- `/permissions` 只读视图；
- journal 记 decision（若事件模型允许扩展）。
...
### 13.4 与现有代码接合点（供后续实现参考，本次不改代码）

| 点 | 说明 |
| --- | --- |
| `internal/tools/command.go` | `runCommand` 前 `Evaluate`；deny 返回软结果 |
| `internal/config` | `tools.run_command.approval/policy/workspace_only` |
| `internal/tui` | 审批 UI、`/permissions`、状态栏 |
| `internal/store` | 可选：记录 policy 决策/模式变更事件 |
| `cmd/eino-assistant` | CLI flag 覆盖 approval |
