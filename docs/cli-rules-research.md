...
#### Phase 2 — `run_command` 策略与审批

行为：

1. 结构化 policy 决策；默认从 `approval: on_request` 或至少对高危 `ask` 开始（产品可默认更宽，但文档要响）。
2. TUI 审批：一次允许 / 会话允许 / 拒绝。
3. `workspace_only` 默认 true。
4. 状态栏展示 `cmd=ask|auto`。
5. journal 记录审批事件（可审计）。

非目标：OS 级 seatbelt/bwrap（可列为 Phase 3）。
...
| 文档 | 类型 | 内容 |
| --- | --- | --- |
| `docs/cli-rules-research.md` | 调研（本文） | 生态、取舍、架构、阶段 |
...
`docs/context-compaction-research.md` 已强调：不可丢的是指令、权限、当前目标。[14]
...
14. 本仓库，`docs/context-compaction-research.md`、`docs/session-persistence.md`。  
