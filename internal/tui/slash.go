...
func slashCatalog() []slashCommand {
	return []slashCommand{
		...
		{Name: "/agent", Description: "start one bounded read-only background analysis agent (optional diff snapshot)", NeedsArg: true},
...
func helpText() string {
	return strings.Join([]string{
		...
		"  /agent [--diff] <task>  start a bounded read-only background analysis agent; --diff adds a quoted Git snapshot",
