...
func slashCatalog() []slashCommand {
	return []slashCommand{
		...
		{Name: "/agent", Description: "start one bounded read-only background analysis agent (optional diff or file snapshot)", NeedsArg: true},
		{Name: "/agents", Description: "list queued/running agents, inspect, append, or cancel one/all", NeedsArg: true},
		...
		{Name: "/fork", Description: "fork current, selected, latest, or a searchable saved-session picker", NeedsArg: true},
		{Name: "/delete", Description: "permanently delete a saved session after explicit confirmation", NeedsArg: true},
...
func helpText() string {
	return strings.Join([]string{
		...
		"  /agent [--diff|--file path]... <task>  start a bounded read-only agent with optional quoted diff or file snapshots",
		"  /agents [show <id>|append <id>|cancel <id>|cancel all]  list queued/running agents, inspect, draft, or cancel",
		...
		"  /fork [id|name|--last|--pick]  fork current, selected, newest, or picker-chosen session at its latest committed turn (auto child ID)",
		"  /delete <id|name> --yes  permanently delete an inactive saved session (--force also confirms)",
